...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    cudaMalloc(&device_x, array_size);
    cudaMalloc(&device_y, array_size);
    cudaMalloc(&device_result, array_size);
    ...
    //
    cudaMemcpy(device_x, xarray, array_size, cudaMemcpyHostToDevice);
    cudaMemcpy(device_y, yarray, array_size, cudaMemcpyHostToDevice);
    ...
    double overallKernelDuration = kernelEndTime - kernelStartTime;
    printf("Effective BW by CUDA saxpy (kernel only): %.3f ms\t\t[%.3f GB/s]\n", 1000.f * overallKernelDuration, GBPerSec(totalBytes, overallKernelDuration));
